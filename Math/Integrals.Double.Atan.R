...
### History
# - [refactor] DIFF( ATAN ) moved to file:
...
### SQRT:
# Note: sqrt(x*y) can be trivially converted to (x*y);

### I( atan(sqrt(x*y)) )
integrate(\(x) sapply(x, \(y) integrate(\(x)
	atan(sqrt(x*y)), 0, 1, rel.tol=1E-12)$value), 0, 1, rel.tol=1E-12)
pi/2 + 2*Catalan - 3;

### I( x * atan(sqrt(x*y)) )
integrate(\(x) sapply(x, \(y) integrate(\(x)
	x * atan(sqrt(x*y)), 0, 1, rel.tol=1E-12)$value), 0, 1, rel.tol=1E-12)
pi/2 - 4/3;
...
### I( atan(sqrt(x*y)) / sqrt(x) )
integrate(\(x) sapply(x, \(y) integrate(\(x)
	atan(sqrt(x*y)) / sqrt(x), 0, 1, rel.tol=1E-12)$value), 0, 1, rel.tol=1E-12)
2 - 2*log(2);
...
Catalan/2 + asinh(1)*pi * 3/8 - (digamma(7/8) - digamma(1/8)) / 8 + 1/4;
...
# Other:
integrate(\(x) sapply(x, \(y) integrate(\(x)
	1/x * atan(sqrt(x*(2*y-x))), 0, y, rel.tol=1E-10)$value), 0, 1, rel.tol=1E-11)
Catalan + pi/2 * asinh(1) - (digamma(5/8) - digamma(3/8)) / 2 - pi/4 + log(2)/2;

#
integrate(\(x) sapply(x, \(y) integrate(\(x)
	y/x * atan(sqrt(x*(2*y-x))), 0, y, rel.tol=1E-10)$value), 0, 1, rel.tol=1E-11)
integrate(\(x) sapply(x, \(y) integrate(\(x)
	y/x * atan(y/x*sqrt((2*x-1))), 1, Inf, rel.tol=1E-10)$value), 0, 1, rel.tol=1E-11)
Catalan/2 + 3/8 * pi * asinh(1) - (2 - sqrt(2)) * pi/4 +
	- (digamma(5/8) - digamma(3/8)) * 3/8 + 1/4;


### I( atan(sqrt(y^2-x^2)) / (x+y) )
integrate(\(x) sapply(x, \(y) integrate(\(x)
	atan(sqrt(y^2-x^2)) / (x+y), 0, y, rel.tol=1E-11)$value), 0, 1, rel.tol=1E-11)
pi/2 * asinh(1) - (digamma(5/8) - digamma(3/8)) / 2 - Catalan + pi/4 - log(2)/2;

### I( x * atan(sqrt(y^2-x^2)) / (x+y) )
integrate(\(x) sapply(x, \(y) integrate(\(x)
	x * atan(sqrt(y^2-x^2)) / (x+y), 0, y, rel.tol=1E-11)$value), 0, 1, rel.tol=1E-11)
Catalan/2 - (asinh(1)/2 + 2 - sqrt(2)) * pi/4 + (digamma(5/8) - digamma(3/8)) / 8 + 1/4;

### I( y * atan(sqrt(y^2-x^2)) / (x+y) )
integrate(\(x) sapply(x, \(y) integrate(\(x)
	y * atan(sqrt(y^2-x^2)) / (x+y), 0, y, rel.tol=1E-11)$value), 0, 1, rel.tol=1E-11)
3/8 * asinh(1) * pi - Catalan/2 - (digamma(5/8) - digamma(3/8)) / 8 - 1/4;


### I( atan(sqrt(y^2-x^2)) / (y^2 - x^2) )
integrate(\(x) sapply(x, \(y) integrate(\(x)
	atan(sqrt(y^2-x^2)) / (y^2-x^2), 0, y, rel.tol=1E-10)$value), 0, 1, rel.tol=1E-10)
# TODO

### I( x * atan(sqrt(y^2-x^2)) / (y^2 - x^2) )
integrate(\(x) sapply(x, \(y) integrate(\(x)
	x * atan(sqrt(y^2-x^2)) / (y^2-x^2), 0, y, rel.tol=1E-10)$value), 0, 1, rel.tol=1E-10)
Catalan - pi/4 + log(2)/2;

### I( y * atan(sqrt(y^2-x^2)) / (y^2 - x^2) )
integrate(\(x) sapply(x, \(y) integrate(\(x)
	y * atan(sqrt(y^2-x^2)) / (y^2-x^2), 0, y, rel.tol=1E-10)$value), 0, 1, rel.tol=1E-10)
pi/2 * asinh(1) - (digamma(5/8) - digamma(3/8)) / 2;

### I( x*y * atan(sqrt(y^2-x^2)) / (y^2-x^2) )
integrate(\(x) sapply(x, \(y) integrate(\(x)
	x*y * atan(sqrt(y^2-x^2)) / (y^2-x^2), 0, y, rel.tol=1E-11)$value), 0, 1, rel.tol=1E-11)
Catalan/2 - pi/8 + 1/4;


### I( atan(sqrt(y^2-x^2)) / sqrt(y^2-x^2) )
integrate(\(x) sapply(x, \(y) integrate(\(x)
	atan(sqrt(y^2-x^2)) / sqrt(y^2-x^2), 0, y, rel.tol=1E-10)$value), 0, 1, rel.tol=1E-10)
(digamma(7/8) - digamma(5/8)) * (digamma(3/8) - digamma(1/8)) / 8;

### I( x * atan(sqrt(y^2-x^2)) / sqrt(y^2-x^2) )
integrate(\(x) sapply(x, \(y) integrate(\(x)
	x * atan(sqrt(y^2-x^2)) / sqrt(y^2-x^2), 0, y, rel.tol=1E-10)$value), 0, 1, rel.tol=1E-10)
1/2 - log(2)/2;

### I( y * atan(sqrt(y^2-x^2)) / sqrt(y^2-x^2) )
integrate(\(x) sapply(x, \(y) integrate(\(x)
	y * atan(sqrt(y^2-x^2)) / sqrt(y^2-x^2), 0, y, rel.tol=1E-10)$value), 0, 1, rel.tol=1E-10)
(digamma(7/8) - digamma(3/8)) * (digamma(5/8) - digamma(1/8)) / 32 - sqrt(2)/2 * log(sqrt(2)+1) + 1/2;
...
### I( atan(y-x) / (2 - x - y) )
integrate(\(x) sapply(x, \(y) integrate(\(x)
	atan(y-x) / (2 - x - y), 0, y, rel.tol=1E-13)$value), 0, 1, rel.tol=1E-13)
integrate(\(x) 1/2 * (2-x) * log(2-x) / (x^2+1), 0, 1, rel.tol=1E-13)$value - pi^2 / 96;
# TODO
...
- pi^3 / 32 - pi^2 / 24 + pi/2 - log(2) + Catalan * log(2);
...
integrate(\(x) sapply(x, \(y) integrate(\(x)
	x * atan(x*y) / (1 + x - x*y), 0, 1, rel.tol=1E-13)$value), 0, 1, rel.tol=1E-13)
integrate(\(x) sapply(x, \(y) integrate(\(x)
	atan(x) / (2 - x - y), 0, 1, rel.tol=1E-13)$value), 0, 1, rel.tol=1E-13)$value +
	+ 5/96 * pi^2 + pi*log(2)/8 - log(2)^2 / 8 + log(2)/2 - pi/4 - Catalan;
integrate(\(x) (2-x) * log(2-x) / (x^2+1), 0, 1, rel.tol=1E-13)$value + log(2)/2 - pi/4;
# TODO
...
integrate(\(x) sapply(x, \(y) integrate(\(x)
	atan(x*y) * log(1+x), 0, 1, rel.tol=1E-13)$value), 0, 1, rel.tol=1E-13)
33/64 * pracma::zeta(3) - pi^2 / 96 - (2*pi*Catalan - 3*pi*log(2) + 2*pi + log(2)^2 - 4*log(2)) / 8;
...
### I( atan(1-x) / (x^2+y^2) )
integrate(\(x) sapply(x, \(y) integrate(\(x)
	(pi/4 - atan(1-x)) / (x^2+y^2), 0, 1, rel.tol=1E-13)$value), 0, 1, rel.tol=1E-13)
integrate(\(x) atan(x) * atan(1-x) / x, 0, 1)$value +
	+ Catalan*pi/4 - pi^2 *log(2) / 16;
# TODO

### I( x * atan(1-x) / (x^2+y^2) )
integrate(\(x) sapply(x, \(y) integrate(\(x)
	x * atan(1-x) / (x^2+y^2), 0, 1, rel.tol=1E-12)$value), 0, 1, rel.tol=1E-12)
integrate(\(x) - atan(x) * atan(1-x), 0, 1)$value + pi/4 * (pi/2 - log(2));
# TODO

### I( y * atan(1-x) / (x^2+y^2) )
integrate(\(x) sapply(x, \(y) integrate(\(x)
	y * atan(1-x) / (x^2+y^2), 0, 1, rel.tol=1E-12)$value), 0, 1, rel.tol=1E-12)
integrate(\(x) 1/2 * log(x^2 + 1) * atan(1-x), 0, 1)$value +
	- (5/96*pi^2 + pi*log(2)/8 - log(2)^2 / 8 + log(2)/2 - pi/4 - Catalan);
# TODO

### I( x*y * atan(1-x) / (x^2+y^2) )
integrate(\(x) sapply(x, \(y) integrate(\(x)
	x*y * atan(1-x) / (x^2+y^2), 0, 1, rel.tol=1E-13)$value), 0, 1, rel.tol=1E-13)
integrate(\(x) 1/2 * x * atan(1-x) * log(x^2+1), 0, 1)$value + 
	- (5/96 * pi^2 - log(2)^2 / 8 + log(2)/4 - 3/4);
# TODO

### I( x^2 * atan(1-x) / (x^2+y^2) )
integrate(\(x) sapply(x, \(y) integrate(\(x)
	x^2 * atan(1-x) / (x^2+y^2), 0, 1, rel.tol=1E-12)$value), 0, 1, rel.tol=1E-12)
integrate(\(x) -1/2 * atan(x) * atan(1-x), 0, 1)$value - pi*log(2)/4 + pi/4;
# TODO

### I( y^2 * atan(1-x) / (x^2+y^2) )
integrate(\(x) sapply(x, \(y) integrate(\(x)
	y^2 * atan(1-x) / (x^2+y^2), 0, 1, rel.tol=1E-12)$value), 0, 1, rel.tol=1E-12)
integrate(\(x) 1/2 * atan(x) * atan(1-x), 0, 1)$value + pi*log(2)/4 - log(2)/2;
# TODO
...
### I( atan(x) / (x^2*y^2 + 1) )
integrate(\(x) sapply(x, \(y) integrate(\(x)
	atan(x) / (x^2*y^2 + 1), 0, 1, rel.tol=1E-12)$value), 0, 1, rel.tol=1E-12)
- 7/8 * pracma::zeta(3) + pi*Catalan / 2;

### I( x * atan(x) / (x^2*y^2 + 1) )
integrate(\(x) sapply(x, \(y) integrate(\(x)
	x * atan(x) / (x^2*y^2 + 1), 0, 1, rel.tol=1E-12)$value), 0, 1, rel.tol=1E-12)
pi^2 / 16 + pi*log(2)/4 - Catalan;

### I( y * atan(x) / (x^2*y^2 + 1) )
integrate(\(x) sapply(x, \(y) integrate(\(x)
	y * atan(x) / (x^2*y^2 + 1), 0, 1, rel.tol=1E-12)$value), 0, 1, rel.tol=1E-12)
5/96 * pi^2 - pi*log(2)/8 - log(2)^2 / 8;
...
### I( 1/x * atan(x) / (x^2*y^2 + 1) )
integrate(\(x) sapply(x, \(y) integrate(\(x)
	1/x * atan(x) / (x^2*y^2 + 1), 0, 1, rel.tol=1E-12)$value), 0, 1, rel.tol=1E-12)
- pi^2 / 16 + pi/4 * log(2) + Catalan;

### I( y/x * atan(x) / (x^2*y^2 + 1) )
integrate(\(x) sapply(x, \(y) integrate(\(x)
	y/x * atan(x) / (x^2*y^2 + 1), 0, 1, rel.tol=1E-12)$value), 0, 1, rel.tol=1E-12)
Catalan / 2 - pi * log(2) / 8 + pi/8 - log(2)/4;


### I( x * atan((1-x)/y) / (x^2 + y^2) )
integrate(\(x) sapply(x, \(y) integrate(\(x)
	x * atan((1-x)/y) / (x^2+y^2), 0, 1, rel.tol=1E-13)$value), 0, 1, rel.tol=1E-13)
integrate(\(x) - x * atan(x)*atan(1-x), 0, 1)$value + pi^2/8;
...
### I( x*y * atan(x/y) / sqrt(1-x*y) )
integrate(\(x) sapply(x, \(y) integrate(\(x)
	 x*y * atan(x/y) / sqrt(1-x*y), 0, 1, rel.tol=1E-13)$value), 0, 1, rel.tol=1E-13)
(5/3 - 2*log(2)) * pi/3;
