...
x^4*(x+b0)^2*(x^2 + 3*(k1-k2)*(x+b0)) * d2y +
	- x^2 * (4*x^5 + 2*(4*b0+4*k1-5*k2)*x^4 +
		+ (4*b0^2 + 22*b0*k1 - 26*b0*k2 - 3*k1^2 + 3*k2^2)*x^3 +
	...
	+ (56/9 * x^6 + 2*(6*b0 + 4*k1 - 7*k2)*x^5 + # DIV by 9;
		...
		+ (18*b0^2*k1 - 24*b0^2*k2 - 22*b0*k1^2 + 4*b0*k1*k2 + 20*b0*k2^2 + 3*k1^2*k2 - 3*k1*k2^2)*x^3 +
		+ (6*b0^3*k1 - 6*b0^3*k2 - 20*b0^2*k1^2 + 19*b0^2*k2^2 + 2*b0^2*k1*k2 +
			+ 9*b0*k1^2*k2 - 9*b0*k1*k2^2)*x^2 +
		- (6*b0^3*k1^2 - 6*b0^3*k2^2 - 9*b0^2*k1^2*k2 + 9*b0^2*k1*k2^2)*x +
...
### Case: k1 = k2
x = sqrt(3); k = 2/5; b0 = 2/3; c0 = -1/2; # b0 = k
params = list(x=x, k1=k, k2=k, b0=b0, c0=c0);
e = expression(x^2 * (x + b0)^(1/3) * exp(k1/x) + x^2 * (x + b0)^(2/3) * exp(k2/x) + c0)[[1]];
#
y   = eval(e, params);
dy  = eval(D(e, "x"), params);
...
### ODE:
x^4*(x+b0)^2 * d2y +
	- 2*x^2 * (2*x^3 + (4*b0-k)*x^2 + 2*b0*(b0 - k)*x - k*b0^2) * dy +
	+ (56/9 * x^4 + 2*(6*b0 - 3*k)*x^3 + # DIV by 9;
		+ (6*b0^2 - 12*k*b0 + k^2)*x^2 +
		- 2*k*b0*(3*b0 - k)*x + b0^2*k^2) * (y - c0) # = 0
