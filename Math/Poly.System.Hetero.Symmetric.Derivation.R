...
### Sum =>
x^5 + y^5 + b*(x+y) - 2*R # = 0
(x+y)*(x^4 + y^4 - x*y*(x^2 + y^2) + (x*y)^2 + b) - 2*R
S*(S^4 - 4*x*y*S^2 + 2*(x*y)^2 - x*y*(S^2 - 2*x*y) + (x*y)^2 + b) - 2*R
S*(S^4 - 5*x*y*S^2 + 5*(x*y)^2 + b) - 2*R
...
25*S^10 - 75*x*y*S^8 + 25*(x*y)^2*S^6 - 25*b*S^6
25*S^10 - 15*S^5*(2*S^5 - 3*b*S + R) + (2*S^5 - 3*b*S + R)^2 - 25*b*S^6
5*S^10 - (2*S^5 - 3*b*S + R)^2 - 20*b*S^6 + 15*R*S^5
5*S^10 - (4*S^10 + 9*b^2*S^2 + R^2 - 12*b*S^6 + 4*R*S^5 - 6*b*R*S) - 20*b*S^6 + 15*R*S^5
### Eq:
S^10 - 8*b*S^6 + 11*R*S^5 - 9*b^2*S^2 + 6*b*R*S - R^2 # = 0


### Classic Polynomial:
print.p(div.pm(classic.S2Simple.gen(5), classic.BaseSimple.gen(5))[[1]])
...
### Diff =>
x^5 - y^5 + b4*(x^4 - y^4) - b3*(x - y)*(S^2 - x*y) - b2*(x - y)*S - b1*(x-y) # = 0
(x-y)*(x^4 + y^4 + b4*S*(S^2 - 2*x*y) - b3*(S^2 - x*y) + x*y*(x^2 + y^2) + (x*y)^2 - b2*S - b1) # = 0
# Case: x != y =>
...
S^10 + 4*b4*S^9 - 2*(b3 - 3*b4^2)*S^8 - 4*(b2 + b3*b4 - b4^3)*S^7 +
	...
	+ ((3*b2 - 15*b3*b4 + 4*b4^3)*R - 9*b1^2 + 6*b2^2*b3 + 4*b1*b3^2 +
...
### "Complementary Poly"
# [NOT the roots / false roots]
(b1^3) +
(3*b1^2*b2)*x^1 +
(3*b1*b2^2)*x^2 +
(3*b1^2 - 2*b1^2*b3^2 + b2^3)*x^3 +
(6*b1*b2 - 3*b1*b2*b3^2)*x^4 +
(- R*b3^3 + 3*b2^2 - b2^2*b3^2)*x^5 +
(3*b1 - 4*b1*b3^2 + b1*b3^3 + b1*b3^4)*x^6 +
3*b2*(1 - b3^2)*x^7 +
(1 - b3^2)^2 * x^9
...
-b1/S * (Z^2 - 2*x*y) + b1*S - 2*R # = 0
-b1*(S^2 - 2*x*y) + b1*Z^2 - 2*R*S # = 0
2*b1*x*y - 2*R*S
R*S - b1*x*y
R*S + b1^2/S
R*S^2 + b1^2
...
(x - y)*((x*y)^3 + b1) # = 0
# Case: x != y
# x*y = (-b1)^(1/3) * m3.all; where m3^3 = 1;

### Sum =>
(x*y)^3*(x+y) + 2*b2*(x+y)^2 + b1*(x+y) - 2*R # = 0
