...
m = matrix(c(5,-4,0,0,-5,  1,0,0,20,5,  1,0,20,10,1,  1,20,10,6,1,  21,10,6,6,1), ncol=5)
s = solve(m, c(0,0,0,0,1))
s * det(m)
# 1 / px(x)
1/px(x)
sum(sapply(r, function(r) sum(s * r^c(0:4)) / (x - r) ))


### Ex 2:
bb = sqrt(c(11,7,5,3,2)); bb[2] = - bb[2];
b0 = bb[1]; b1 = bb[2]; b2 = bb[3]; b3 = bb[4]; b4 = bb[5];
bb = c(bb, 1);
r  = roots(rev(bb));
px = function(x) sum(bb * x^(0:5));
x = 3;
#
sum(1/(x - r))
(5*x^4 + 4*b4*x^3 + 3*b3*x^2 + 2*b2*x + b1) / px(x)
#
sum(r/(x - r))
(- b4*x^4 - 2*b3*x^3 - 3*b2*x^2 - 4*b1*x - 5*b0) / px(x)
#
sum(r^2/(x - r))
((b4^2 - 2*b3)*x^4 + (b3*b4 - 3*b2)*x^3 + (b2*b4 - 4*b1)*x^2 + (b1*b4 - 5*b0)*x + b0*b4) / px(x)
#
sum(r^3/(x - r))
((- b4^3 + 3*b3*b4 - 3*b2)*x^4 - (b3*b4^2 - 2*b3^2 - b2*b4 + 4*b1)*x^3 +
	- (b2*b4^2 - 2*b2*b3 - b1*b4 + 5*b0)*x^2 - (b1*b4^2 - 2*b1*b3 - b0*b4)*x +
	- (b0*b4^2 - 2*b0*b3)) / px(x)
#
sum(r^4/(x - r))
((b4^4 - 4*b3*b4^2 + 4*b2*b4 + 2*b3^2 - 4*b1)*x^4 +
	# (b4^4    - b3*b4^2 + b2*b4 - 3*b3*b4^2  + 3*b2*b4 + 2*b3^2 - 4*b1)*x^4 +
	+ (b3*b4^3 - b2*b4^2 + b1*b4 - 3*b3^2*b4  + 3*b2*b3 + 2*b2*b3 - 5*b0)*x^3 +
	+ (b2*b4^3 - b1*b4^2 + b0*b4 - 3*b2*b3*b4 + 3*b2^2  + 2*b1*b3)*x^2 +
	+ (b1*b4^3 - b0*b4^2 + 0     - 3*b1*b3*b4 + 3*b1*b2 + 2*b0*b3)*x +
	+ (b0*b4^3 - 0       + 0     - 3*b0*b3*b4 + 3*b0*b2 + 0)) / px(x)
#
