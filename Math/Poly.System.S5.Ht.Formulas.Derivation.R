...
max.pow.S(c("E3V131", "E3Vn131", "E3V531", "E3Vn531"), pow=2, FUN=f2, skip.path=T, R=c(1,2.8,1,0,2), R2=c(5,3,1,0,2))
max.pow.S(c("E3V111", "E3V1n11", "E3V131", "E3V1n31"), pow=2, FUN=f2, skip.path=F, npos=2, R=c(1.2,1,0.9,0,2), R2=c(0.9,3,1.2,0,2))


solve.coeff(c(1,1,1,0,2), c(1.2,1,1.1,0,2), c(...) / 2,
"c(...)", function(R) { Rn = R; Rn[3] = - Rn[3]; (f2(R) + f2(Rn))/2; })
...
x = list(c(4.6,1.1,0.9,0,2), c(-5/4,3/4,5/4,0,2), c(1.3,3,1,0,2.1), c(1,2.8,1,0,2), c(5,3,1,0,2))
m = sapply(x, fd)
v = c(...) - sapply(x, f2)
solve(t(m), v)
...
27*(E11a^7 + E11b^7)*E5^2 +
	...
	- (E11a*E11b)^2*(6*E5^2*S^6 + 750*E5^3*S + 375*E3^2*E5^2) +
	# x^1:
...
f2 = function(R) {
	...
	cc = 14*E5^3*S^5 - 21*E3^2*E5^2*S^4 + 8*E3^4*E5*S^3 - E3^6*S^2 + 500*E3*E5^3*S^2 +
		...
		+ E11a*(200*E5^3*S^3 - 2*E3^6 + 625*E3*E5^3) +
		- E11a^2*(6*E5^2*S^6 + 750*E5^3*S + 375*E3^2*E5^2) +
