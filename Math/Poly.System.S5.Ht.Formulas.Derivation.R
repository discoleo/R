...
solve.coeff(c(1,3,1,0,2), c(1.2,3,1,0,2), cb / 4,
"c(...)", function(R) { Rn = R; Rn[3] = - Rn[3]; f2e(R)/4; })
...
f2e = function(R) {
	R1n = R; R1n[1] = - R1n[1]; R2 = R; R2[2] = - R2[2]; R2n = R2; R2n[1] = - R2n[1];
	f2(R) - f2(R2) + (f2(R1n) - f2(R2n));
}

cb = c(...)
x = list(c(1,3,1,0,2), c(1.2,3,1,0,2), c(0.9,3,1.2,0,2))
m = sapply(x, fd)
v = cb - sapply(x, f2e)
solve(t(m), v/4)
...
27*(E11a^7 + E11b^7)*E5^2 +
	...
	- E11a*E11b*(28*E5^3*S^5 + 34*E3^5*E5 + 3*5^5*E5^4) +
	# B0:
