...
gcd = function(x) {
	len = length(x);
	if(len == 2) {
		return(pracma::gcd(x[1], x[2]));
	} else if(len <= 1) return(x);
	x0 = abs(x[1]);
	sapply(x[-1], function(x) pracma::gcd(x0, x));
}
...
make.poly(15)[1:8, 5]; c(1001,-429,-869,-704,-249,251,621,756) / sqrt(6466460);
make.poly(16)[1:8, 5]; c( 273, -91,-221,-201,-101, 23,129,189) / sqrt(470288);
...
(70*nc^4 - 140*(n+1) * nc^3 + 10*(9*n^2 + 21*n + 17) * nc^2 +
	- 10*(2*n^3 + 9*n^2 + 17*n + 10) * nc +
	+ (n^4 + 10*n^3 + 35*n^2 + 50*n + 24)) / 24 / sqrt(div);


###
nc = seq(5); # n = 9;
(70*nc^4 - 1400*nc^3 + 9350*nc^2 - 23500*nc + 17160) / (24*5);
nc = seq(5); # n = 10;
(70*nc^4 - 1540*nc^3 + 11270*nc^2 - 30800*nc + 24024) / (24*7);
nc = seq(6); # n = 11;
(70*nc^4 - 1680*nc^3 + 13370*nc^2 - 39480*nc + 32760) / (24*35);
nc = seq(6); # n = 12;
(70*nc^4 - 1820*nc^3 + 15650*nc^2 - 49660*nc + 43680) / (24*10);
nc = seq(7); # n = 13;
(70*nc^4 - 1960*nc^3 + 18110*nc^2 - 61460*nc + 57120) / (24*5);
nc = seq(7); # n = 14;
(70*nc^4 - 2100*nc^3 + 20750*nc^2 - 75000*nc + 73440) / (24*5);
nc = seq(8); # n = 15;
(70*nc^4 - 2240*nc^3 + 23570*nc^2 - 90400*nc + 93024) / 24;
nc = seq(8); # n = 16;
(70*nc^4 - 2380*nc^3 + 26570*nc^2 - 107780*nc + 116280) / (24*5);

x = c(70,28,154,616,2002,2860,286,8008,68068,136136,6466460,470288)
x = x * c(1,25,25,25,25,49,35^2,100,25,25,1,25)
n = seq(5, 16)
diff(x) |> diff() |> diff() |> diff() |> diff() |> diff() |> diff() |> diff()
