...
# - Taq polymerase is NOT completely resistant against
#   inactivation: t(1/2) ~ 40 min. at 95 Celsius;
# - In many curves on the web: slower curves increase
#   beyond the faster curves: this is theoretically impossible!
plot.quantity = function(k = 1/12, lwd = 2, cex = c(2, 1.5),
	...
	if(length(k) > 1) {
		...
		b = k[1]; a = b * c(3/2, 1, 2/3);
	}
	par.old = par(mar = c(5, 5, 2, 1.5) + 0.1)
	curve(funGr(x, a = a[1], b=b), xlim = c(0, 40), ylim = c(0, 1),
		xlab = "PCR Cycles", ylab = "Amplified DNA", col = col[1],
		lwd = lwd, cex.lab = cex[1], cex.axis = cex[2]);
	curve(funGr(x, a = a[2], b=b), lwd = lwd, add = TRUE, col = col[2]);
	curve(funGr(x, a = a[3], b=b), lwd = lwd, add = TRUE, col = col[3]);
	if(! is.null(h))
